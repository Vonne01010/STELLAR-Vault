    ...
    <div className="rounded-2xl bg-white border border-slate-100 p-5 text-[#1A1A1A] space-y-4 animate-fadeIn">
      {!publicKey ? (
        ...
        <div className="space-y-3">
          ...
          {sendMode === 'amount' ? (
            <>
              {/* Input is always available now — not gated by existing pending transfers */}
              ...
              {/* List every pending transfer involving this user, sender or receiver */}
              {pendingApprovals.length > 0 && (
                <div className="space-y-3 pt-1">
                  {pendingApprovals.map((pendingApproval) => (
                    <div key={pendingApproval.id} className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 space-y-3 text-[11px] animate-fadeIn">
                      ...
                      <div className="flex gap-2 pt-1">
                        {pendingApproval.sender === publicKey && !pendingApproval.senderAuthorized && (
                          <button type="button" onClick={() => onApproveAsSender(pendingApproval.id)} disabled={busy} className="flex-1 py-2.5 rounded-xl bg-linear-to-r from-[#FF9F1C] to-[#F37A00] text-white text-[10px] uppercase tracking-wider disabled:opacity-50">
                            Sign Sender
                          </button>
                        )}
                        {pendingApproval.recipient === publicKey && !pendingApproval.receiverAuthorized && (
                          <button type="button" onClick={() => onApproveAsReceiver(pendingApproval.id)} disabled={busy} className="flex-1 py-2.5 rounded-xl bg-linear-to-r from-[#FF9F1C] to-[#F37A00] text-white text-[10px] uppercase tracking-wider disabled:opacity-50">
                            Sign Receiver
                          </button>
                        )}
                        {pendingApproval.sender === publicKey && pendingApproval.senderAuthorized && pendingApproval.receiverAuthorized && (
                          <button type="button" onClick={() => onSubmitApprovedTransfer(pendingApproval.id)} disabled={busy} className="flex-1 py-2.5 rounded-xl bg-linear-to-r from-[#FF9F1C] to-[#F37A00] text-white text-[10px] uppercase tracking-widest font-normal disabled:opacity-50">
